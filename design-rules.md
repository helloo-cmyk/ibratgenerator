...
### Rule 2: The Brat Aesthetic is Absolute
* **Strict Guideline**: The site follows the visual style of Charli XCX's 2024 album "Brat".
* **Design Language**: It is intentionally imperfect, unapologetically loud, brutalist, and completely unbothered. Do not try to make it look "pretty", "clean", or "corporate". Complexity kills it.

### Rule : Typography & Font Rules
* **Strict Guideline**: Standard text must be sans-serif (Arial/Helvetica style).
* **Headings**: Must use ultra-bold, italic, tightly tracked typography. Use `font-black italic uppercase tracking-tighter` or `lowercase`.
* **Standard Classes**: Always use the predefined homepage design system classes when creating new sections:
  * `.hp-display-heading` for H2 titles.
  * `.hp-body-text` for paragraph text (with `opacity-70` for secondary text).
* **Serifs**: Only permitted for very specific decorative numerals (e.g., `01`, `02`) to contrast with the brutalist sans-serif.
